...
- **Template**: estende `base.html`, layout `col-md-5` (formulário) + `col-md-7` (resultado, com placeholder ilustrado quando `resultado` é `None`). O gráfico é um SVG desenhado à mão via JS vanilla (sem Chart.js) — copie o bloco `<style>`+`<script>` de `templates/simuladores/juros_compostos.html` quase 1:1; os dados chegam via `{{ resultado.pontos_grafico|json_script:"pontos-grafico" }}` no formato `[{mes, saldo}, ...]` (saldo em `float`, não `Decimal`, pois é o único ponto onde o JS precisa do valor). Para simuladores de tom de alerta (dívida, por exemplo), troque a paleta do gráfico de `--series-1`/`--series-1-fill` para `var(--fq-rust)`; para simuladores de "crescimento" mantenha a paleta azul padrão.
  - **Legibilidade do gráfico em qualquer tela (não mexa sem entender o porquê)**: o `viewBox` do SVG **não** é fixo em `800x320` — o script mede o tamanho real renderizado (`svg.getBoundingClientRect()`) e ajusta o `viewBox` para bater com ele antes de desenhar. Isso faz o `font-size` dos rótulos (classe `.rotulo`, `13px`) corresponder a pixels de tela de verdade em qualquer largura de card, em vez de encolher proporcionalmente num card estreito de mobile (era assim antes, e o texto ficava ilegível a ~4px). Duas consequências que também precisam ser copiadas junto: (1) o número de marcações do eixo X é calculado a partir da largura disponível (`numTicksXDesejado`, com base na largura medida do rótulo mais largo via `medidor.measureText`) em vez de um `6` fixo, senão os rótulos "Mês N" se sobrepõem em telas estreitas; (2) o último rótulo do eixo X usa `text-anchor: end` (não `middle`) para não ser cortado na borda do `viewBox`, e o penúltimo rótulo é removido quando não sobra espaço entre os dois. Se criar um simulador com um eixo/rótulo diferente de "Mês N", reaproveite a mesma lógica de medição em vez de valores fixos.
